  ...
  \begin{columns}[T]
    \begin{column}{0.4\textwidth}
      \begin{tcolorbox}[bloqueverde, title=Contexto]
        \small
        \begin{itemize}
        \item \textbf{Siniestralidad vial:} Desafío crítico de salud pública.
        \item \textbf{Diferenciador:} Análisis de coocurrencia y combinación de factores específicos.
        \item \textbf{Objetivo:} Identificar patrones concurrentes de alto riesgo.
        \end{itemize}
      \end{tcolorbox}
    \end{column}
...
\begin{frame}[plain]
  \frameheader{03}{Metodología}
  \myfooter
      \begin{tcolorbox}[bloqueverde, title=Pruebas de independencia $\chi^2$]
        \small
        \begin{itemize}
      \item \textbf{Chi-cuadrado:} evalúa la independencia entre variables \parencite{LASTRE2019}.
        \[
        \chi^2=\sum_i\sum_j
        \frac{(n_{ij}-E_{ij})^2}{E_{ij}}
        \]
          $$ H_0: \text{Las variables son independientes vs } H_1: \text{ Las variables se relacionan}$$
        \item \textbf{V de Crámer:} mide la fuerza de la asociación \parencite{McHugh2013}.
        \[
        V=\sqrt{\frac{\chi^2}{N\min(I-1,J-1)}}
        \]

        \item \textbf{Residuos estandarizados:} identifican las combinaciones responsables de la asociación \parencite{Agresti2002}.
        \[
        r_{ij}=
        \frac{n_{ij}-E_{ij}}
...
\begin{frame}[plain]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}
\item \textbf{Rural:} pico en la mañana (rutina agrícola temprana).
\item \textbf{Urbano:} pico en la noche (ocio, salida laboral, poca visibilidad).
...
\begin{frame}[plain]
  \frameheader{03}{Conclusiones}
  \myfooter
      \begin{tcolorbox}[bloqueverde, title= Conclusiones]
        \small
        \begin{itemize}
    \item Se identificaron las combinaciones de factores asociadas a una mayor frecuencia de 
    accidentes.
    \item Mayor número de colisiones en condiciones favorables por alta exposición vehicular.
    \item La gran muestra produjo asociaciones débiles, aunque estadísticamente significativas.
    \item \textbf{Residuos estandarizados}: Patrones de riesgo específicos (alta velocidad, 
    superficies mojadas y oscuridad).
    \item Se recomienda el uso de modelos multivariados en estudios futuros.
    \item El estudio amplía la evidencia a nivel nacional y aporta información para mejorar la 
    seguridad vial.
        \end{itemize}
      \end{tcolorbox}
\end{frame}
