...
% Elimina las líneas del encabezado y pie
...
\begin{center}
\textbf{Análisis de combinaciones de entorno en accidentes de tránsito del Reino Unido (2010-2017) mediante pruebas Chi-Cuadrado de independencia, V de Crámer y residuos estandarizados}
\end{center}

\vspace{0.5em}
\noindent Debbie Con Ortega, Ashly Garro Villanueva, Emily Sanchéz Mancía, Alessandro Umaña Vega\footnote{Estudiantes de Ciencias Actuariales, Universidad de Costa Rica.}\\
debbie.con@ucr.ac.cr, ashly.garro@ucr.ac.cr, emily.sanchezmancia@ucr.ac.cr, alessandro.umana@ucr.ac.cr
...
Los accidentes de tránsito representan un desafío relevante para la salud pública 
  ...
  variables del entorno son evaluadas mediante 3 métodos: las pruebas de independencia 
...
\noindent \textbf{Palabras clave:} Siniestralidad vial, Factores de riesgo, Análisis estadístico
...
Para abordar el estudio de la siniestralidad vial en el Reino 
Unido se usarán las metodologías expuestas en las literaturas investigadas. 
Se trabajarán diferentes enfoques para lograr el objetivo del trabajo, donde 
se usará la prueba de independencia de Chi-cuadrado y el análisis de los residuos 
estandarizados. También se complementa con el tratamiento previo de la información, 
abarcando el cálculo del coeficiente V de Crámer para medir la fuerza de asociación y 
los algoritmos de imputación para el manejo de valores faltantes. 
Esta estructura permite entrelazar las herramientas teóricas con los objetivos de 
la investigación de una manera coherente y ordenada.

El análisis exploratorio de datos se desarrolla con el objetivo 
primordial de estudiar qué patrones o combinaciones de factores ocurren 
con mayor frecuencia en los accidentes de tránsito en el Reino Unido durante 
el periodo comprendido entre los años 2010 y 2017, utilizando como base empírica 
el registro gubernamental UK Road Safety: Traffic Accidents and Vehicles. A través 
de esta perspectiva, el estudio busca no solo identificar patrones recurrentes, sino 
también reconocer de forma objetiva situaciones de riesgo que fundamenten la propuesta 
de encontrar factores que se relacionan para reducir la frecuencia de estos siniestros.
...
se incorporaron las condiciones de iluminación, la condición 
climática, la condición de la superficie de la calle, la zona del accidente, la hora de ocurrencia 
y la velocidad límite de la vía.

En cuanto al tratamiento inicial y depuración de la información se optó por 
eliminar los datos con la información desconocida o que no cumplían con el 
propósito de garantizar el cumplimiento de los supuestos matemáticos. Asimismo, se descartaron los datos que 
no lograban asegurar la validez de los contrastes de hipótesis, estos registros indeterminados fueron 
excluidos únicamente durante la fase de ejecución de las pruebas 
Chi-cuadrado y el cálculo de los residuos estandarizados. También, se ejecutó un proceso 
técnico de imputación selectiva para los valores nulos estructurados como NA. Estos fueron 
reemplazados utilizando la media aritmética para variables numéricas, y la moda estadística 
para aquellas variables cualitativas o categóricas \parencite{USECHE2006}. 
Este enfoque balanceado permite conservar la mayor cantidad posible de información 
original sin introducir sesgos artificiales que perjudiquen las estimaciones posteriores.

Para identificar de forma precisa las combinaciones y patrones de 
factores ambientales e infraestructurales que se presentan con mayor 
frecuencia en los siniestros viales del Reino Unido, se estructuró un diseño de 
análisis específico para variables categóricas. Este procedimiento 
se basa en el uso de tablas de contingencia, contrastes de hipótesis de 
independencia y la evaluación de la V de Crámer y residuos estandarizados. Según \textcite{LASTRE2019}, 
la distribución conjunta de dos variables cualitativas puede estudiarse de forma 
rigurosa mediante representaciones tabulares matriciales. Si se define $X$ y $Y$ como 
dos variables categóricas con $I$ y $J$ categorías respectivamente, la tabla de 
contingencia de orden $(I \times J)$ agrupará las frecuencias observadas para cada 
par posible de categorías interconectadas. Sea $n_{ij}$ la frecuencia absoluta 
observada correspondiente a la celda que cruza la categoría $i$ de la variable $X$ con 
la categoría $j$ de la variable $Y$. El número total de observaciones ($N$) muestrales del 
modelo se denota matemáticamente como:

$$
N = \sum_{i}\sum_{j} n_{ij}
$$

A partir de estos valores observados, es factible calcular 
la proporción conjunta de cada combinación mediante 
la relación $p_{ij} = \dfrac{n_{ij}}{N}$, describiendo 
formalmente el comportamiento de la muestra para luego 
inspeccionar sus distribuciones.

Una vez completado el paso de tabulación, se aplica la 
prueba Chi-cuadrado de independencia de Pearson con el fin de determinar si 
existe una relación estadísticamente significativa entre los factores seleccionados. 
El principio operativo de esta prueba radica en comparar las frecuencias observadas 
en cada celda con las frecuencias teóricas esperadas que se obtendrían bajo el supuesto 
estricto de que las variables no guardan ninguna relación de dependencia. Bajo la hipótesis 
nula de independencia ($H_0$), las frecuencias esperadas se determinan:

$$
E_{ij} = \frac{n_{i+}\, n_{+j}}{N}
$$

Donde $n_{i+}$ representa el total de la fila $i$ y $n_{+j}$ representa el total de la 
columna $j$. Utilizando estos componentes, el estadístico de contraste Chi-cuadrado global 
se define:
...
Para el propósito de esta investigación, la hipótesis nula $H_0$ postula 
que los factores relacionados con los accidentes de tránsito son independientes 
entre sí, mientras que la hipótesis alternativa $H_1$ afirma que existe una relación 
de dependencia o asociación entre dichos factores. El estadístico calculado sigue una 
distribución teórica Chi-cuadrado con grados de libertad equivalentes a $(I-1)(J-1)$. Además, el p-valor 
se extrae como la probabilidad de obtener un valor estadístico más extremo que el observado 
bajo la validez de la hipótesis nula. Fijando un nivel de significancia de $\alpha =$ 0.05, 
cualquier p-valor inferior a este umbral forzará el rechazo de la independencia, concluyendo 
la presencia de una asociación real entre las variables del entorno.

No obstante, esta prueba solo determina si existe una relación entre las variables, 
pero no muestra su magnitud. Por lo que se introduce la V de Crámer como medida complementaria de 
intensidad. Este coeficiente, diseñado para corregir el efecto de las dimensiones de 
la tabla ante tamaños de muestra muy grandes, se calcula como:
$$
V = \sqrt{\frac{\chi^2}{N \cdot min(I-1, J-1)}}
$$
Esta métrica oscila estrictamente en el intervalo entre 0 y 1, donde los valores próximos 
al cero muestran una asociación de fuerza débil y los valores cercanos a uno evidencian 
que se relacionan mucho entre sí \parencite{McHugh2013}. Esto posibilitará comparar 
de manera directa la relevancia de las distintas combinaciones de factores sobre la 
base de datos.

Finalmente, se usa el análisis de residuos estandarizados para resolver las 
limitaciones del estadístico global, el cual no detalla cuáles combinaciones 
específicas de categorías son las mayores causales de la frecuencia de siniestros. 
De esta forma, se evalúa el aporte individual de cada celda a la asociación, dividiendo 
la diferencia entre la frecuencia observada y la esperada entre la raíz cuadrada de su 
varianza, ajustada por el producto de los complementos de las proporciones marginales de 
su respectiva fila y columna \parencite{Agresti2002}.

$$
r_{ij} = \frac{n_{ij} - E_{ij}}{\sqrt{E_{ij}(1-p_{i+})(1-p_{+j})}}
$$

Para su correcta interpretación, se adopta el criterio
 estadístico que establece que valores absolutos de residuos 
 superiores a dos o tres indican desviaciones significativas frente al 
 modelo teórico independiente. Los residuos con signo negativo representan 
 combinaciones que ocurren con menor frecuencia de la esperada de forma aleatoria. 
 Por el contrario, los residuos positivos que superan los valores críticos de dos o 
 tres identifican los patrones de riesgo más importantes en el Reino Unido. 
 Esto revela cuáles combinaciones de factores ambientales e infraestructurales son 
 realmente críticas y requieren una intervención preventiva.
...
Los análisis exploratorios iniciales revelan que la frecuencia vial en el Reino 
  ...
  cantidad absoluta de colisiones viales. Esto coincide con los hallazgos de 
  ...
  resultados (note Tabla \ref{tab:resumen_chicuadrado}) muestran que ambas variables no
  son independientes. El análisis de residuos estandarizados, mostrado a continuación 
  en la Figura \ref{fig:residuos sup limvel} indicó que los accidentes ocurridos en superficies
 ...
 \newpage
  \noindent\textbf{Figura 1}
    ...
    \noindent \textit{Residuos de condición de superficie y límite de velocidad}
  \begin{figure}[H]
          \centering
          \includegraphics[width=0.52\textwidth]{Figuras/Residuos_superficie_lim.png}
  ...
  mostrando en la Tabla \ref{tab:resumen_chicuadrado} una 
  fuerza de asociación débil. 
  Los residuos estandarizados 
  positivos, mostrados en la Figura 
  \ref{fig:residuos_interseccion_obstaculos}, evidencian que las superficies resbaladizas debido a derrames accidentales, 
  ...
  \noindent \textit{Residuos de detalle de intersección y condiciones especiales de la vía}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.52\textwidth]{Figuras/Residuos_intresecc_obstaculos.png}
        \captionsetup{labelformat=empty,textformat=empty}
        \caption{}
  ...
  Por otra parte, se evalúa la asociación entre el área donde ocurre el siniestro y el momento
  del día. La prueba de Chi-cuadrado determinó una relación de dependencia con un 
  p-valor menor a 0.05, observe la Tabla \ref{tab:resumen_chicuadrado}, y una 
  fuerza de asociación moderada. El análisis de los residuos 
  estandarizados en la Figura
...
\noindent \textit{Residuos de zona (urbana o rural) y momento del día}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.52\textwidth]{Figuras/Residuos_horaZona.png}
        \captionsetup{labelformat=empty,textformat=empty}
        \caption{}
  ...
  El siguiente resultado analiza 
  la interacción entre la condición de la superficie de la vía y la condición de la 
  luz, note la Tabla \ref{tab:resumen_chicuadrado}. Al aplicar la prueba de independencia 
  Chi-cuadrado a estas variables categóricas, se obtuvo un valor p menor a 0.05, 
  ...
  su magnitud es débil debido a la cantidad de datos usados.
  ...
  de colisión. No obstante, al aislar los residuos 
  positivos significativos, se 
  descubren patrones locales importantes 
  observados a continuación en la Figura \ref{fig:residuos superfice luz},
  ...
  \noindent Residuos condición de superficie de la vía y condición luz
  \begin{figure}[H]
        \centering
  ...
  Por otra parte, la evaluación entre las condiciones específicas de la superficie
  y el tipo de carretera, donde la prueba de Chi-cuadrado y el coeficiente V de
  Crámer mostradas en la Tabla \ref{tab:resumen_chicuadrado} denotan que hay una dependencia, 
  pero al la V de Crámer estar cercana a 0 no es tan fuerte. Al evaluar los residuos 
  ...
  \noindent \textit{Residuos de condición de superficie y tipo de vía}
\begin{figure}[H]
    \centering
    \captionsetup{labelformat=empty,textformat=empty}
    \caption{} 
...
\end{figure}
  ...
  Por último, el análisis de la interacción entre la condición de iluminación y tipo de vía mediante la 
  prueba Chi-cuadrado reveló una dependencia significativa, observe la Tabla \ref{tab:resumen_chicuadrado}
  (p-valor = 2.2 $\times 10^{-16}$, V de Crámer = 0.004974), demostrando 
  mediante los residuos estandarizados que las carreteras de vías dobles en oscuridad 
  total registran accidentes muy superiores a los esperados,
   mostrado en la Figura \ref{fig:residuo iluminacion via}.
  ...
  \noindent \textit{Residuos de condición de luz y tipo de vía}
    \begin{figure}[H]
        \centering
...
\hspace{5.5mm}El presente estudio permitió identificar combinaciones de factores ambientales e 
 infraestructurales asociadas con una mayor frecuencia de accidentes de tránsito en 
 el Reino Unido durante el periodo 2010-2017. Los resultados mostraron que, aunque la 
 mayor cantidad de accidentes ocurrió en condiciones aparentemente favorables, como 
 superficies secas, clima despejado y buena iluminación, este comportamiento se explica 
 por la mayor exposición vehicular en dichos escenarios y no necesariamente por un mayor nivel de riesgo. En contraste, el análisis de los residuos estandarizados permitió identificar aquellas combinaciones cuya frecuencia fue significativamente superior a la esperada bajo independencia, destacando la interacción entre altas velocidades y superficies mojadas o con hielo, así como la presencia de oscuridad total en vías dobles. Estos hallazgos evidencian que la probabilidad de ocurrencia de un accidente no depende de un único factor, sino de la interacción entre múltiples condiciones del entorno.

Asimismo, la aplicación conjunta de la prueba de independencia Chi-cuadrado, el coeficiente V de Crámer y el análisis de residuos estandarizados permitió no solo establecer la existencia de asociaciones entre variables categóricas, sino también identificar cuáles combinaciones específicas son las principales responsables de dichas asociaciones. Aunque la mayoría de los coeficientes V de Crámer reflejaron asociaciones de baja intensidad, los resultados muestran que incluso relaciones débiles pueden revelar patrones relevantes cuando se analizan bases de datos de gran tamaño, aportando información valiosa para orientar estrategias de prevención y fortalecer la toma de decisiones en materia de seguridad vial.

Finalmente, el estudio amplía la evidencia disponible en la literatura al analizar información correspondiente a todo el territorio del Reino Unido durante un periodo de ocho años, superando investigaciones previas enfocadas en regiones o escenarios específicos. Los resultados respaldan la importancia de considerar simultáneamente diversos factores ambientales e infraestructurales al estudiar la siniestralidad vial y constituyen una base útil para el diseño de políticas públicas orientadas a reducir la frecuencia y severidad de los accidentes de tránsito.

Como limitaciones de la investigación, la gran cantidad de observaciones provocó que la prueba Chi-cuadrado produjera valores de significancia extremadamente pequeños, mientras que los coeficientes V de Crámer indicaron asociaciones de baja intensidad, lo que dificulta valorar la relevancia práctica de algunas relaciones identificadas. En futuras investigaciones, esta limitación podría abordarse mediante el uso de modelos multivariados, como modelos lineales generalizados o técnicas de aprendizaje automático, que permitan evaluar de manera conjunta el efecto de múltiples variables y cuantificar con mayor precisión su influencia sobre la ocurrencia de accidentes. Asimismo, el tratamiento de los valores faltantes mediante métodos simples de imputación constituye otra limitación del estudio, ya que puede introducir incertidumbre en las estimaciones. Como trabajo futuro, se recomienda implementar técnicas de imputación más robustas, como la imputación múltiple o métodos basados en algoritmos de aprendizaje, con el fin de mejorar la calidad de los datos y aumentar la confiabilidad de los resultados obtenidos.

\seccion{AGRADECIMIENTOS}

 Nos gustaría expresar nuestro más sincero agradecimiento al Dr. Maikol Solis por su orientación, apoyo y acompañamiento durante el desarrollo de esta investigación. Sus conocimientos, observaciones y recomendaciones fueron fundamentales para la correcta aplicación de las metodologías planteadas y fortalecer el análisis realizado.

\newpage
\printbibliography
\vspace{1.2em}
  \begin{center}\textbf{ANEXOS}\end{center}
  \vspace{0.3em}

\noindent Todo el código realizado para lograr los resultados se encuentra en el siguiente \href{https://github.com/CA303-I-2026/Grupo-2.git}{Repositorio de GitHub}.\\

\noindent\textbf{Tabla 1}
  
\noindent \textit{Resumen pruebas Chi-cuadrado de independencia y V de Crámer}
